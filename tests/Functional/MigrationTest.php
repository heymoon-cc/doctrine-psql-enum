...
class MigrationTest extends TestCase
{
    ...
    public function testUpdateSchema()
    {
        ...
        $connection->executeQuery('DROP TABLE IF EXISTS dangling');
        ...
        $expected = [
            ...
            'CREATE TABLE HasEnumEntity (id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL, "time" example NOT NULL, another another_example NOT NULL, PRIMARY KEY (id));',
            "CREATE TYPE order_status AS ENUM ('pending','completed','cancelled');",
            'CREATE TABLE "order" (id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL, status order_status NOT NULL, PRIMARY KEY (id));',
            "CREATE TABLE custom.custom_schema (id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL, example example NOT NULL, PRIMARY KEY (id));",
        ];
        ...
        $command->run(new ArrayInput(['--dump-sql' => true]), $output);
        $sql = explode(PHP_EOL, $output->fetch());
        $this->assertEquals('ALTER TABLE hasenumentity ADD another another_example NOT NULL;', array_shift($sql));
        // Test dropping column
        $connection->executeQuery('ALTER TABLE HasEnumEntity ADD unexisting example');
        $connection->executeQuery('CREATE TABLE dangling (id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL, unexisting example)');
        $command = $application->get('doctrine:schema:update');
        $command->run(new ArrayInput(['--dump-sql' => true]), $output);
        $sql = explode(PHP_EOL, $output->fetch());
        $this->assertContains('ALTER TABLE hasenumentity DROP unexisting;', $sql);
        $this->assertContains('DROP TABLE dangling;', $sql);
    }
}
